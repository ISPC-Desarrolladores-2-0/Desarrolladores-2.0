...
print("BIENVENIDO AL SISTEMA DE CONSULTAS DE LEYES")
...
#CLASE CONSULTA Y SUS 5 ATRIBUTOS (DEFINE UNA ESTRUCTURA BASICA PARA CREAR OBJETOS (CONSULTA))DESARROLLADO POR MARCO VIRINNI
class Consulta:
...
def mostrar_leyes(conexion):
    ...
    if len(leyes) == 0:
        print("No hay leyes para mostrar.")
    else:
        print("\n=== LISTA DE LEYES ===\n")
...
def eliminar_ley(conexion, id_ley):
    ...
    if ley is None:
        print("Nro de ley inválido.")
    else:
        cursor.execute("DELETE FROM leyes WHERE nro_ley = ?", (id_ley,))
        conexion.commit()
        print(f"Ley eliminada: N° {ley[0]}")
...
def submenu_leyes(conexion):
    while True:
        ...
        if opcion == "1":
            ...
            mostrar_leyes(conexion)
            id_ley = int(input("Ingrese el N° de Ley a borrar: "))
...
def buscar_leyes(conexion):
    ...
    if opcion_busqueda.lower() == "n":
        nro_ley = input("Ingrese el número de ley: ")
...
def cargar_leyes(conexion):
    ...
    for ley in leyes:
        ...
        if existing_ley is None:
            ...
            print(f"Ley {ley[0]} cargada exitosamente.")
...
def submenu_principal(conexion_leyes, conexion_usuarios, conexion_consultas):
    while True:
        ...
        print("5. Editar lista de consultas")
